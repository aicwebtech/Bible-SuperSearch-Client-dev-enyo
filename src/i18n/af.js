module.exports = {
    meta: {
        // 2 or 3 character language code, lower-case, ie 'es'
        // Note: This filename name must be [code].js ie 'es.js'
        code: 'af',

        // Native name of language (endonymn): ie 'Español'
        name: 'Afrikaans',    

        // English name of language, ie 'Spanish'
        nameEn: 'Afrikaans',       

        // Whether language is read Right-to-left
        isRtl: false,           
       
        // true if done by person fluent in both English and this language, false if using translation software
        isFluent: false,     

        // Name of individulal or software used to translate
        translatedBy: 'Google Translate', 
        
        // Email address of translator, if fluent (not software)
        translatedEmail: null, 
    },
    
    // The actual translation data

    // Notes:
    // * Please place the corresponding translation text between '' (single quotes) on each line below.
    // * Please maintain capitalization and punctuation, if possible.
    // * Any ' (single quote) in the translation MUST be preceded by a \ (backslash).

    'Bible SuperSearch': 'Bybel Super Soek',
    'Loading, please wait': 'Laai tans, wag asseblief',

    // Search Forms
    'Select Bible version(s)': 'Kies Bybel weergawe(s)',
    'Search': 'Soek',
    'Search the Bible': 'Soek die Bybel',
    'Go': 'Gaan',
    'Bible Search': 'Bybel Soek',
    'Bible': 'Bybel',
    'Whole Words Only': 'Heel Woorde Slegs',
    'Whole words only': 'Slegs hele woorde',
    'Whole words': 'Hele woorde',
    'Exact Case': 'Presiese Geval',
    'Parallel Bible #': 'Parallelle Bybel #',
    'Select a Bible': 'Kies \'n Bybel',
    'Add Bible': 'Voeg Bybel by',
    'Remove Bible': 'Verwyder Bybel', 
    'Random Chapter': 'Ewekansige Hoofstuk',
    'Random Verse': 'Willekeurige Vers',
    'Passages': 'Passasies',
    'Passage': 'Deurgang',
    'The book of': 'Die boek van',
    'Look for': 'Soek vir',
    'Look up': 'Kyk op',
    'in': 'in',
    'verse': 'vers',
    'verses': 'verse',
    'A Chapter': 'n Hoofstuk',
    'Chapters': 'Hoofstukke',
    'Find verses containing': 'Vind verse wat bevat',
    'Restrict search to': 'Beperk soektog tot', 
    'Word Search': 'Woord Soek',
    'Clear Form': 'Vee vorm uit',
    'Select Book and Chapter:' : 'Kies Boek en Hoofstuk:',
    'Select Book, Chapter, Verse:' : 'Kies Boek, Hoofstuk, Vers:',

    // Context Links
    'Show full chapter': 'Wys volledige hoofstuk',
    'Chapter': 'Hoofstuk',
    'Show this verse': 'Wys hierdie vers',
    'Show in context': 'Wys in konteks',
    'Context': 'Konteks',
    'In Context': 'In Konteks',
    'Cross References': 'Kruisverwysings',
    'Cross Reference': 'Kruisverwysing',
    'Original Verse': 'Oorspronklike vers',
    'Open All': 'Maak Alles Oop',
    'Hidden': 'Versteek',
    'Toggle': 'Wissel',
    'Show': 'Wys',
    'Cross Reference Format': 'Kruisverwysingsformaat',
    'Compact': 'Kompak',
    'Auto': 'Outomaties',
    'Expand': 'Uitgebrei',

    'Limit Search To': 'Beperk Soek Na',
    'Limit search to': 'Beperk soektog na', // Watch case!
    'Entire Bible': 'Hele Bybel',
    'Passage(s) listed below': 'Passasie(s) hieronder gelys',
    'Passage(s) listed above': 'Passasie(s) hierbo gelys',
    
    // Shortcuts
    'Old Testament': 'Ou Testament',
    'New Testament': 'Nuwe Testament',
    'Law': 'Wet',
    'History Books': 'Geskiedenis Boeke', // Can translate as 'History'
    'History': 'Geskiedenis',
    'Wisdom & Poetry': 'Wysheid en Poësie',
    'Prophets Books': 'Profete Boeke', // Can translate as 'Prophets'
    'Prophets': 'Profete',
    'Major Prophets': 'Majoor Profete',
    'Minor Prophets': 'Klein Profete',
    'Gospels': 'Evangelies',
    'Epistles': 'Briewe',
    'Pauline Epistles': 'Pauliniese Briewe',
    'General Epistles': 'Algemene Briewe',
    'End Times Prophecy': 'Eindtyd profesie',
    
    // Shortcut Aliases
    'OT': 'OT',
    'NT': 'NT',
    'Torah': 'Torah',
    'Wisdom': 'Wysheid',
    'Major': 'Majoor',
    'Minor': 'Klein',
    'Passion': 'Passie',
    'Doctrine': 'Leerstelling',
    'Paul': 'Paul',
    'General': 'Algemeen',
    'Last Days': 'Laaste Dae',

    'Look up Passage(s)': 'Soek deurgang(e) op',
    'Enter passage(s)' : 'Voer deurgang(s) in',
    'Enter word(s), phrase(s) or expression(s)' : 'Voer woorde), frases) of uitdrukkings in)',
    'Example': 'Voorbeeld',

    'Search for' : 'Soek vir',
    'Match': 'Wedstryd',
    'All Words': 'Alle Woorde',
    'Any Word': 'Enige Woord',
    'Exact Phrase': 'Presiese Frase',
    'Only One Word': 'Slegs Een Woord',
    'Two or More Words': 'Twee of Meer Woorde',
    'Words Within 5 Verses': 'Woorde Binne 5 Verse',
    'Words Within Same Chapter': 'Woorde Binne Dieselfde Hoofstuk',
    'Words Within Same Book': 'Woorde Binne Dieselfde Boek',
    'Boolean Expression': 'Boole Uitdrukking',
    'Regular Expression': 'Gereelde Uitdrukking',
    'Select one ...' : 'Kies een...',

    'Enter search keyword(s) or passage reference(s)': 'Voer soek sleutelwoord(s) of deurgangsverwysing(s) in',
    'Tip: Entering both a passage and a search query will result in the limitation of the search to the specified passage(s).': 'Wenk: Om beide \'n gedeelte en \'n soeknavraag in te voer, sal lei tot die beperking van die soektog na die gespesifiseerde gedeelte(s).',
    
    // Advanced Search
    'Boolean Search': 'Boolese Soek',
    'Passage Retrieval': 'Passasie Herwinning',
    'Find words within': 'Vind woorde binne',
    'Proximity Search': 'Nabyheid Soek',
    'Power Search': 'Krag Soek',

    // <b> ... </b> is HTML for bold text, please ensure that your translation maintains the bold
    '<b>All</b> of the words' : '<b>Alle</b> van die woorde',
    '<b>Any</b> of the words' : '<b>Enige</b> van die woorde',
    '<b>Only one</b> of the words': '<b>Slegs een</b> van die woorde',
    '<b>None</b> of the words' : '<b>Geen</b> van die woorde',
    'The <b>exact phrase</b>': 'Die <b>presiese frase</b>',

    'Bible #': 'Bybel #', // downloads

    // Pagination
    'Next Page' : 'Volgende Bladsy',
    'Previous Page' : 'Vorige Bladsy',
    'First Page' : 'Eerste Bladsy',
    'Last Page': 'Laaste Bladsy',
    'Page': 'Bladsy',
    
    // Formatting and dialog buttons
    'A': 'A',     // First letter in given language
    'Abc': 'Abc',  // First 3 letters in given language, with 1st capitalized
    'Serif' : 'Serif',                     // * Button text, please keep translation short!
    'Sans-Serif': 'Sans-Serif',                 // * Button text, please keep translation short!
    'Monospace': 'Mono ruimte',                  // * Button text, please keep translation short!
    'Enlarge Text': 'Vergroot Teks',
    'Default Text Size': 'Verstek Teksgrootte',
    'Shrink Text': 'Krimp Teks',
    'Verse Display': 'Vers Vertoon',
    'Paragraph Display': 'Paragraaf Vertoon',
    'Passage Display': 'Deurgang Vertoon', 
    'Verse as Passage Display': 'Vers soos Deurgang Display', 
    'EZ Copy': 'Maklik Kopie',  // 'Easy Copy'
    'Read Display': 'Lees Vertoon',
    'Italics': 'Kursief',                    // * Button text, please keep translation short!
    'Disable Italization of Added Words' : 'Skakel Kuralisasie van Bygevoegde Woorde',
    'Enable Italization of Added Words': 'Aktiveer Kurisering van Bygevoegde Woorde',
    'Disable Strong\'s Numbers': 'Skakel Strong se nommers uit',
    'Enable Strong\'s Numbers': 'Aktiveer Strong se nommers',
    'Red Letter': 'Rooi Brief',
    'Disable Red Letter': 'Skakel Rooi Brief uit',
    'Enable Red Letter': 'Aktiveer Rooi Brief',
    'Highlight': 'Hoogtepunt',                   // * Button text, please keep translation short!
    'Disable Highlighting of Keywords': 'Skakel Hoogtepunte van Sleutelwoorde',
    'Enable Highlighting of Keywords': 'Aktiveer die klem van Sleutelwoorde',
    'Hide Parallel Search Errors': 'Verberg Parallelle Soek Foute', // 5.6.20
    'Show Parallel Search Errors': 'Wys Parallelle Soek Foute', // 5.6.20
    'Print': 'Druk',
    'Printer Friendly': 'Drukker Vriendelik',
    'Help': 'Help',
    'Basic': 'Basies',
    'Advanced': 'Gevorderde',

    // Descriptions of special formatting features
    'Formatting': 'Formatering',  
    'Supported Bibles Only': 'Slegs Ondersteun Bybels', 
    
    // Translate the right-side strings here
    'strongs_numbers_description': 'Strong se konkordansienommers dui die onderliggende Hebreeuse of Griekse woorde aan.',  
    'italics_description': 'Kursief dui woorde aan wat in vertaling bygevoeg is wat nie in die oorspronklike taal was nie.',  
    'red_letter_description': 'Woorde van Christus in Rooi.', 
    'highlight_description': 'Verlig (vet) sleutelwoorde in soektogte.', 

    // Verse Cherry-picking
    'Select All': 'Kies Alle', 
    'Select None': 'Kies Geen', 
    'Select': 'Kies', 
    'Unselect': 'Ontkies', 
    
    // Copy format options
    'Copy Format': 'Kopieer Formaat', 
    'Word Processor': 'Woordverwerker', 
    'Outline' : 'Omlyn', 
    'Custom': 'Pas', 

    'Single Verses': 'Enkel Verse', 
    'Reference - Text': 'Verwysing - Teks', 
    'Text - Reference': 'Teks - Verwysing', 
    'Text Only': 'Slegs Teks', 
    'Separate Line': 'Aparte Lyn', 

    'Multi-verse Passages': 'Multi-vers Passasies',
    'Reference - Passage': 'Verwysing - Passasie',
    'Passage - Reference': 'Passasie - Verwysing', 
    'Passage Only': 'Slegs Deurgang', 
    'Show Verse Number': 'Wys Vers Nommer', 

    'Other': 'Ander', 
    'Abbreviate Books': 'Afkort Boeke', 
    'Include Testament': 'Sluit Testament in', // v5.6.0
    'Testament': 'Testament', // v5.6.0
    'Omit Extra Line Breaks': 'Laat ekstra lynbreuke weg',

    // Settings Dialog
    'Settings' : 'Instellings', 
    'Language' : 'Taal', 
    'Text Display': 'Teks Vertoon', 
    'Font Style': 'Font Styl',
    'This will reset the settings to defaults.':'Dit sal die instellings terugstel na verstek.', // 5.6.20
    'Do you want to save the settings for future use?' :'Wil jy die instellings stoor vir toekomstige gebruik?',
    'Some Bibles may not be available for download due to copyright restrictions.' :'Sommige Bybels is dalk nie beskikbaar vir aflaai nie as gevolg van kopieregbeperkings.',

    // Audio Bible
    'Listen': 'Luister', // 6.2
    'Audio Bible may have been generated by using AI, and not an actual human voice.': 'Die oudiobybel is moontlik gegenereer deur KI te gebruik, en nie \'n werklike menslike stem nie.', // 6.2
    'Audio not available for this passage.': 'Oudio nie beskikbaar vir hierdie gedeelte nie.', // 6.2

    // Result and error messages
    
    // Your search produced (number of verses) results
    'Your search produced' : 'Jou soektog het geproduseer',  
    // (total number of verses)
    'results' : 'resultate',
    
    // Showing results [1] to [30]
    'Showing Results': 'Wys resultate',
    'to' : 'aan',
    
    // Your search produced no results in [name of Bible version]
    'Your search produced no results in': 'Jou soektog het geen resultate opgelewer nie', 
    'However, verses from this Bible have been included for comparison.': 'Vers uit hierdie Bybel is egter ter vergelyking ingesluit.',
    
    // Operators such as [boolean operator] cannot be at the (beginning|end) of your search
    'Operators such as': 'Operateurs soos',
    'cannot be at the beginning of your search.': 'kan nie aan die begin van jou soektog wees nie.',
    'cannot be at the end of your search.': 'kan nie aan die einde van jou soektog wees nie.',
    'Please remove it, or use it\'s lower case equivalent.': 'Verwyder dit asseblief, of gebruik dit se kleinletter-ekwivalent.',

    // Your search for [attempted keywords] is invalid...
    'Your search for': 'Jou soektog na',
    'is invalid, and appears to be a passage reference.': 'is ongeldig, en blyk \'n deurgangsverwysing te wees.',
    'is invalid.': 'is ongeldig',

    // Your search was limited to [number of] verses
    'Your search was limited to': 'Jou soektog was beperk tot',
    'Please refine your search if necessary': 'Verfyn asseblief jou soektog indien nodig',

    'This Bible is in the Public Domain.': 'Hierdie Bybel is in die Publieke Domein.',
    'This Bible is provided for research purposes only.': 'Hierdie Bybel word slegs vir navorsingsdoeleindes verskaf.',
    'An unknown error has occurred.' : '\'n Onbekende fout het voorgekom.',
    'An error has occurred, please try again later.': '\'n Fout het voorgekom, probeer asseblief later weer.',
    'Error' : 'Fout',
    'Success': 'Sukses',
    'Cache not found': 'Kas is nie gevind nie',
    'No query terms provided. Please specify search term(s) and/or passage reference(s).': 'Geen navraagbepalings verskaf nie. Spesifiseer asseblief soekterm(s) en/of deurgangverwysing(s).',
    'Your search produced no results.' : 'Jou soektog het geen resultate opgelewer nie.',
    'Maximum hits has been reached for today for this domain / IP address': 'Maksimum treffers is bereik vir vandag vir hierdie domein / IP-adres',
    'No Bibles are enabled. Please contact site administrator.': 'Geen Bybels is geaktiveer nie. Kontak asseblief werfadministrateur.',
    'Cannot retrieve multiple books at once.': 'Kan nie veelvuldige boeke gelyktydig ophaal nie.',
    'Bible text not found': 'Bybelteks nie gevind nie',
    'Invalid book in book range': 'Ongeldige boek in boekreeks',
    'Book not found': 'Boek nie gevind nie',
    'Your parentheses are mismatched.': 'Jou hakies is nie ooreenstem nie.',
    'Your parentheses are mismatched, or you have a proximity operator inside of parentheses.': 'Jou hakies is nie ooreenstem nie, of jy het \'n nabyheidsoperateur binne-in hakies.',
    'Proximity operators such as PROX and CHAP can only be used with boolean searches' : 'Nabyheidsoperateurs soos PROX en CHAP kan slegs met Boolese soektogte gebruik word',
    'Note: PROX / CHAP operators cannot be enclosed within parentheses or brackets': 'Let wel: PROX / CHAP operateurs kan nie binne hakies of hakies ingesluit word nie',
    'Your access to this API has been revoked.  Please contact system administrator.': 'Jou toegang tot hierdie API is herroep. Kontak asseblief die stelseladministrateur.',

    // Dialogs - general
    'Close': 'Naby', 

    // Bible SOS dialog
    'Bible SOS': 'Bybel Nood',
    'Emergency Help from the Bible': 'Noodhulp uit die Bybel',
    'Where to go When ...': 'Waarheen om te gaan Wanneer...',
    'Afraid': 'Bang',
    'Anxious': 'Angstig',
    'Backsliding': 'Afkerigheid',
    'Bereaved': 'Bedroefde',
    'Bitter or Critical': 'Bitter of Kritiek',
    'Conscious of Sin': 'Bewus van Sonde',
    'Defeated': 'Verslaan',
    'Depressed': 'Depressief', 
    'Disaster Threatens': 'Ramp Dreig',
    'Discouraged': 'Moedeloos',
    'Doubting': 'Twyfel',
    'Facing a Crisis': 'N Krisis in die gesig staar',
    'Faith Fails': 'Geloof Misluk',
    'Friends Fail': 'Vriende Misluk', 
    'Hopeless': 'Hopeloos',
    'Leaving Home': 'Verlaat Huis',
    'Lonely': 'Eensaam',
    'Needing God\'s protection': 'Benodig God se beskerming',
    'Needing Guidance': 'Benodig Leiding',
    'Needing Peace': 'Nodig Vrede',
    'Needing Rules for Living': 'Benodig Lewensreëls', 
    'Overcome': 'Oorkom',
    'Prayerful': 'Gebeds',
    'Protected': 'Beskerm',
    'Sick or in Pain': 'Siek of in Pyn',
    'Sorrowful': 'Hartseer',
    'Tempted': 'Gelok',
    'Thankful': 'Dankbaar',
    'Traveling': 'Op reis',
    'Trouble, In': 'Moeilikheid, In',
    'Troubled': 'Ontsteld',
    'Weary': 'Moeg',
    'Worried': 'Bekommerd',

    // Bible Start dialog
    'Start': 'Begin',
    'Bible Start Guide': 'Bybel Begin Gids',
    'New to the Bible?': 'Nuut by die Bybel?',
    'Not Sure Where to Begin?' : 'Nie seker waar om te begin nie?',
    'Life and Teachings of Jesus Christ': 'Lewe en Leringe van Jesus Christus',
    'Basic Christian Beliefs': 'Basiese Christelike Oortuigings',
    'Creation and the Fall of Man': 'Skepping en die val van die mens',
    'Birth of Christ / Life of Christ': 'Geboorte van Christus / Lewe van Christus',
    'Imperfection in Churches and Believers': 'Onvolmaaktheid in Kerke en Gelowiges',
    'Early human history, especially that of Israel': 'Vroeë menslike geskiedenis, veral dié van Israel',
    'The relationship between the Old Testament Law and the Church': 'Die verhouding tussen die Ou Testamentiese Wet en die Kerk',
    'History and Law': 'Geskiedenis en Regte',
    'Salvation by faith, not works': 'Verlossing deur geloof, nie werke nie',
    'Christian life and experience. &nbsp;The Apostle Paul gave up credentials under the OT Law to follow Christ': 'Christelike lewe en ervaring. &nbsp;Die apostel Paulus het geloofsbriewe onder die OT-wet prysgegee om Christus te volg',
    'Christ the suffering servant': 'Christus die lydende Dienaar',
    'Christ\'s crucifixion foretold': 'Christus se kruisiging voorspel',

    // Bible downloads dialog
    'Download': 'Laai af',
    'Bible Downloads': 'Bybel Aflaaie',
    'Select Bible(s)' : 'Kies Bybel(s)',
    'Select a Format': 'Kies \'n formaat',
    'Rendering': 'Genereer',
    'Rendering Bibles, this may take some time': 'Om Bybels te genereer, kan dit tyd neem',
    'Rendering is Complete': 'Genereer is Voltooi',
    'Your download should begin shortly': 'Jou aflaai behoort binnekort te begin',
    'If not, please click on the below link': 'Indien nie, klik asseblief op die onderstaande skakel',
    'Manual Download': 'Handleiding Aflaai',
    'Please select at least one Bible': 'Kies asseblief ten minste een Bybel',
    'Please select a format': 'Kies asseblief \'n formaat',
    'Are you sure you want to exit?  This will end the current download.': 'Is jy seker jy wil ophou? Dit sal die huidige aflaai beëindig.',
    'Plain Text' : 'Gewone Teks',
    'Machine-readable Plain Text' : 'Masjienleesbare Gewone Teks',
    'Spreadsheet' : 'Sigblad',
    'Databases': 'Databasisse',

    'PDF - Compact Text, Letter, Words of Christ in Red': 'PDF - Kompakte Teks, Brief, Woorde van Christus in Rooi',
    'PDF - Compact Text, A4, Words of Christ in Red': 'PDF - Kompakte Teks, A4, Woorde van Christus in Rooi',
    'PDF - Compact Text, Letter, Words of Christ Underlined': 'PDF - Kompakte Teks, Brief, Woorde van Christus Onderstreep',
    'PDF - Compact Text, A4, Words of Christ Underlined': 'PDF - Kompakte Teks, A4, Woorde van Christus Onderstreep',

    // Copy / Share / Link dialogs
    'Copy': 'Kopie',
    'Copy Link': 'Kopieer Skakel',
    'Copy with Text': 'Kopieer met Teks',
    'Share Link': 'Deel Skakel',
    'Share' : 'Share',
    'Link' : 'Skakel', 
    'Copied to clipboard': 'Gekopieer na knipbord', 
    'Failed to copy': 'Kon nie kopieer nie', 

    // Quick Start Help Dialog
    'Quick Start Guide': 'Vinnige Begin',
    'User\'s Manual': 'Handleiding gebruiker se',
    'Wildcard': 'Wildkaart',
    'Basic Searches' : 'Basiese Soektogte',
    'Advanced Searches using Boolean': 'Gevorderde Soektogte met Booleaanse gebruik',
    'Select' : 'Kies',
    'Operators': 'Operateurs',
    'Operator': 'Operateur',
    'Aliases': 'Aliases',
    'Description': 'Beskrywing',
    'Match both': 'Vind beide',
    'Match either': 'Vind óf',
    'Match only one': 'Vind net een',
    'Does not match': 'Vind nie',
    'Matches words in the same chapter': 'Vind woorde in dieselfde hoofstuk',
    'Matches words in the same book': 'Vind woorde in dieselfde boek',
    'Matches words in the same book, and within N verses of each other': 'Vind woorde in dieselfde boek, en binne N verse van mekaar',
    'Matches words in the same chapter, and within N verses of each other': 'Vind woorde in dieselfde hoofstuk, en binne N verse van mekaar',
    'For details, please see the User\'s Manual': 'Vir besonderhede, sien asseblief die gebruikershandleiding',
    'Special Proximity Operators': 'Spesiale Nabyheids Operateurs',
    'Matches unlimited characters': 'Wedstryde onbeperkte karakters',
    'searched for': 'soek',

    // Keywords for sample searches
    'Jesus': 'Jesus',
    'Christ': 'Christus',
    'Lord': 'Meester',
    'faith': 'geloof',
    'hope': 'hoop',
    'joy': 'vreugde',
    'preserved' : 'bewaar',
    'preserve' : 'bewaar',
    'stand' : 'staan',
    'word': 'woord',
    'words': 'woorde',
    'truth': 'waarheid',

    // Searching across language

    // Parallel search multi language error messages
    'You cannot search across Bibles of different languages.': 'Jy kan nie deur Bybels van verskillende tale soek nie.',
    'You cannot search across Bibles of different languages with the selected search type.' : 'Jy kan nie deur Bybels van verskillende tale met die geselekteerde soektipe soek nie.',

    // General dialogs
    'Name': 'Naam',
    'Save': 'Stoor',
    'Cancel': 'Kanselleer',
    'Edit': 'Wysig',
    'Delete': 'Skrap',
    'Clear': 'Maak Vorm skoon',
    'Restore': 'Stel terug',
    'Add': 'Voeg',
    'Add to List': 'Voeg by lys',
    'Okay': 'Oukei',
    'Old': 'Oud',
    'New': 'Nuwe',
    'Reset': 'Herstel', // 5.6.20

    'Are you sure?': 'Is jy seker?',
    'Are you sure you want to delete': 'Is jy seker jy wil uitvee',

    // Bookmarks/History
    'This will delete all history.': 'Dit sal alle geskiedenis verwyder.',
    'This will delete all bookmarks.': 'Dit sal alle boekmerke verwyder.',
    'This will clear everything from the verse list.': 'Dit sal alles uit die verslys verwyder.',
    'Move to Current': 'Skuif na Huidig',
    'Move up': 'Skuif op',
    'Move down': 'Skuif af',

    'Bookmark': 'Boekmerk',
    'Bookmarks': 'Boekmerke',
    'Verse List': 'Verslys',
    'Verse list empty': 'Verslys leeg',
    'Add duplicate verse?': 'Voeg duplikaatvers by?',

    'Please delete some bookmarks before adding more.': 'Verwyder asseblief \'n paar boekmerke voordat u meer byvoeg.',
    'This bookmark name already exists.': 'Hierdie boekmerknaam bestaan reeds.',
    'Bookmark name is required.': 'Boekmerk naam word vereis.', // 5.6.20

    // Quick start dialog: Limiting searches
    'Limiting Searches': 'Beperk Soektogte',
    'Passages': 'Passasies',
    'Alias': 'Alias',
    'You cannot search for these common words': 'Jy kan nie vir hierdie algemene woorde soek nie',

    // Custom
    'Tip: To activate chosen Bible versions, look up passage, turn a chapter or execute search.': 'Wenk: Om die gekose Bybelvertalings te aktiveer, soek \'n gedeelte op, blaai na \'n hoofstuk of voer \'n soektog uit.',

    // 5.7:
    'Include Link': 'Sluit skakel in',
    'Include Formatting': 'Sluit Formatering in',
    'Resume Search': 'Hervat Soek',

    // Bible Books
    // For each book in the list below, please specify the (full) name.  Please also provide short name if known.
    // The short name should be the most common abbreviation of the book name in the given language.
    //
    // NOTE: If you are using translation software, please first Google for book lists already translated into the given language.
    // REASON: If we can find the book list already translated, it's probably a better translation than what the software would do.
    // Hint: You can often find these on the leading Bible website gateway when looking for Bibles in the given language.
    // Under avaliable versions, just search for the language, select one of the Bibles that shows up, and copy the Book names.
    //
    // If you cannot find a pre-translated list of the books, then and only then use the translation software to translate them.

    'bibleBooks': [
        {
          "id": 1,
          "name": "Genesis",
          "shortname": ""
        },
        {
          "id": 2,
          "name": "Eksodus",
          "shortname": ""
        },
        {
          "id": 3,
          "name": "Levitikus",
          "shortname": ""
        },
        {
          "id": 4,
          "name": "Numeri",
          "shortname": ""
        },
        {
          "id": 5,
          "name": "Deuteronomium",
          "shortname": ""
        },
        {
          "id": 6,
          "name": "Josua",
          "shortname": ""
        },
        {
          "id": 7,
          "name": "Rigters",
          "shortname": ""
        },
        {
          "id": 8,
          "name": "Rut",
          "shortname": ""
        },
        {
          "id": 9,
          "name": "1 Samuel",
          "shortname": ""
        },
        {
          "id": 10,
          "name": "2 Samuel",
          "shortname": ""
        },
        {
          "id": 11,
          "name": "1 Konings",
          "shortname": ""
        },
        {
          "id": 12,
          "name": "2 Konings",
          "shortname": ""
        },
        {
          "id": 13,
          "name": "1 Kronieke",
          "shortname": ""
        },
        {
          "id": 14,
          "name": "2 Kronieke",
          "shortname": ""
        },
        {
          "id": 15,
          "name": "Esra",
          "shortname": ""
        },
        {
          "id": 16,
          "name": "Nehemia",
          "shortname": ""
        },
        {
          "id": 17,
          "name": "Ester",
          "shortname": ""
        },
        {
          "id": 18,
          "name": "Job",
          "shortname": ""
        },
        {
          "id": 19,
          "name": "Psalms",
          "shortname": ""
        },
        {
          "id": 20,
          "name": "Spreuke van Salomo",
          "shortname": ""
        },
        {
          "id": 21,
          "name": "Prediker",
          "shortname": ""
        },
        {
          "id": 22,
          "name": "Hooglied van Salomo",
          "shortname": ""
        },
        {
          "id": 23,
          "name": "Jesaja",
          "shortname": ""
        },
        {
          "id": 24,
          "name": "Jeremia",
          "shortname": ""
        },
        {
          "id": 25,
          "name": "Klaagliedere van Jeremia",
          "shortname": ""
        },
        {
          "id": 26,
          "name": "Esegiël",
          "shortname": ""
        },
        {
          "id": 27,
          "name": "Daniël",
          "shortname": ""
        },
        {
          "id": 28,
          "name": "Hosea",
          "shortname": ""
        },
        {
          "id": 29,
          "name": "Joël",
          "shortname": ""
        },
        {
          "id": 30,
          "name": "Amos",
          "shortname": ""
        },
        {
          "id": 31,
          "name": "Obadja",
          "shortname": ""
        },
        {
          "id": 32,
          "name": "Jona",
          "shortname": ""
        },
        {
          "id": 33,
          "name": "Miga",
          "shortname": ""
        },
        {
          "id": 34,
          "name": "Nahum",
          "shortname": ""
        },
        {
          "id": 35,
          "name": "Habakuk",
          "shortname": ""
        },
        {
          "id": 36,
          "name": "Sefanja",
          "shortname": ""
        },
        {
          "id": 37,
          "name": "Haggai",
          "shortname": ""
        },
        {
          "id": 38,
          "name": "Sagaria",
          "shortname": ""
        },
        {
          "id": 39,
          "name": "Maleagi",
          "shortname": ""
        },
        {
          "id": 40,
          "name": "Matteus",
          "shortname": ""
        },
        {
          "id": 41,
          "name": "Markus",
          "shortname": ""
        },
        {
          "id": 42,
          "name": "Lukas",
          "shortname": ""
        },
        {
          "id": 43,
          "name": "Johannes",
          "shortname": ""
        },
        {
          "id": 44,
          "name": "Die handelinge van die apostels",
          "shortname": ""
        },
        {
          "id": 45,
          "name": "Romeine",
          "shortname": ""
        },
        {
          "id": 46,
          "name": "1 Korintiërs",
          "shortname": ""
        },
        {
          "id": 47,
          "name": "2 Korintiërs",
          "shortname": ""
        },
        {
          "id": 48,
          "name": "Galasiërs",
          "shortname": ""
        },
        {
          "id": 49,
          "name": "Effesiërs",
          "shortname": ""
        },
        {
          "id": 50,
          "name": "Filippense",
          "shortname": ""
        },
        {
          "id": 51,
          "name": "Kolossense",
          "shortname": ""
        },
        {
          "id": 52,
          "name": "1 Tessalonisense",
          "shortname": ""
        },
        {
          "id": 53,
          "name": "2 Tessalonisense",
          "shortname": ""
        },
        {
          "id": 54,
          "name": "1 Timoteus",
          "shortname": ""
        },
        {
          "id": 55,
          "name": "2 Timoteus",
          "shortname": ""
        },
        {
          "id": 56,
          "name": "Titus",
          "shortname": ""
        },
        {
          "id": 57,
          "name": "Filemon",
          "shortname": ""
        },
        {
          "id": 58,
          "name": "Hebreërs",
          "shortname": ""
        },
        {
          "id": 59,
          "name": "Jakobus",
          "shortname": ""
        },
        {
          "id": 60,
          "name": "1 Petrus",
          "shortname": ""
        },
        {
          "id": 61,
          "name": "2 Petrus",
          "shortname": ""
        },
        {
          "id": 62,
          "name": "1 Johannes",
          "shortname": ""
        },
        {
          "id": 63,
          "name": "2 Johannes",
          "shortname": ""
        },
        {
          "id": 64,
          "name": "3 Johannes",
          "shortname": ""
        },
        {
          "id": 65,
          "name": "Judas",
          "shortname": ""
        },
        {
          "id": 66,
          "name": "Die openbaring",
          "shortname": ""
        }
    ],
};
